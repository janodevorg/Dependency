...
let project = Project(
    ...
    targets: [
        ...
        .target(
            ...
            destinations: [.iPhone, .mac],
            product: .unitTests,
            bundleId: "dev.jano.dependency.test",
    ...
    schemes: [
       Scheme.scheme(
           ...
           buildAction: BuildAction.buildAction(
               targets: [TargetReference.target("Dependency")]
           ),
           testAction: .targets(
               [TestableTarget.testableTarget(target: TargetReference.target("DependencyTests"))],
               configuration: .debug,
               attachDebugger: true
           )
       )
